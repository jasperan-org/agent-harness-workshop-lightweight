...
layer (*what it is · why it matters · what to watch*), then lets you poke the **running** agent and watch the
...
demo streams over Server-Sent Events. The chat model — **OCI Generative AI** — is the only thing reached over the network, and even that is Oracle;
everything else — embeddings, retrieval, memory, the scheduler — runs inside the database.
...
|---|---|---|
| 1 | **Foundation & Models** | Type text → watch it become a 384-dim vector computed *inside* the database; see the loaded models. |
...
3. **An OCI Generative AI API key** (`OCI_GENAI_API_KEY`) — the only outbound call the app makes.
...
> Run all commands below from the **`appbook/`** directory.
...
Open `.env` and set your **OCI GenAI API key** (and the Oracle credentials, if yours differ from the
defaults):
...
LLM_MODEL=xai.grok-4-1-fast-reasoning    # OCI GenAI model id
```
...
`fastapi` / `uvicorn` / `sse-starlette` if they are missing, and starts the server on **http://127.0.0.1:8001**.

<details>
<summary>Prefer to run it manually?</summary>

```bash
# with your environment from step 1 still activated, from the appbook/ directory:
...
Open **http://127.0.0.1:8001** in your browser. The harness warms in a background thread (a few seconds), so
...
Every chapter pairs a clear narrative header (*what it is · why it matters · what to watch*) with a live demo
...
All settings are environment variables (read from `appbook/.env`). Sensible defaults mean you usually only need
to set `OCI_GENAI_API_KEY`.

| Variable | Default | What it is |
|---|---|---|
| `OCI_GENAI_API_KEY` | — | **Required.** Your OCI GenAI key; the only outbound call. |
| `OCI_GENAI_ENDPOINT` | `…us-phoenix-1…oraclecloud.com` | OCI GenAI OpenAI-compatible inference endpoint. |
| `LLM_MODEL` | `xai.grok-4-1-fast-reasoning` | Chat model id on OCI GenAI. |
| `LLM_PROVIDER` | `oci` | `oci` (OCI GenAI) or `openai` (OpenAI direct). |
...
| `HOST` / `PORT` | `127.0.0.1` / `8001` | Where the server listens (env vars for `run.sh`). |
...
| **model** row red / chat errors | `OCI_GENAI_API_KEY` is not set or is invalid. Put it in the app `.env` and restart. |
| `address already in use` on launch | Something is already on port 8001. Stop it, or run on another port: `PORT=8010 ./run.sh`. |
