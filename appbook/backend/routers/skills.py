...
async def tools(req: ToolQuery) -> dict:
    ...
    return {"tools": [{"name": r["NAME"], "description": r["DESCRIPTION"],
                       "schema": r["TOOL_SCHEMA"], "dist": r.get("DIST")} for r in rows]}
...
async def manifest(req: ToolQuery) -> dict:
    ...
    return {"manifest": "\n".join(f"- {r['NAME']}: {r['DESCRIPTION']}" for r in rows),
            "skills": [{"name": r["NAME"], "description": r["DESCRIPTION"],
                        "sha": (r.get("SHA") or "")[:12], "source": r.get("SOURCE_URL")} for r in rows]}
...
async def list_skills() -> dict:
    rows = await run_in_threadpool(db.q,
...
async def create_automation(req: AutomationReq) -> dict:
    return await run_in_threadpool(registries.create_automation, req.name, req.select_sql,
                                   req.cadence_hours, req.description)
