...
@router.get("/health")
def health() -> dict:
    return {"model": settings.model, "api_key_set": bool(settings.llm_api_key),
            "oracle_enabled": settings.oracle_enabled, "harness": db.status()}
...
async def models() -> dict:
    rows = await run_in_threadpool(db.q, "SELECT model_name, mining_function FROM user_mining_models ORDER BY 1")
    return {"models": rows}
...
async def acid_demo() -> dict:
    ...
    def _do():
        ...
        def os_inc():
            for _ in range(300):
                v = int(open(cf).read() or 0); time.sleep(0.0002); open(cf, "w").write(str(v + 1))
        ...
        db.x("MERGE INTO race_counter d USING (SELECT 1 id FROM dual) s ON (d.id=s.id) "
...
async def retrieval(req: SearchReq) -> dict:
    ...
    out = [{"content": str(h["CONTENT"]), "dist": h.get("DIST"), "score": h.get("SCORE"),
            "rrf": h.get("rrf"), "rerank_score": h.get("rerank_score")} for h in hits]
