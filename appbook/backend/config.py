...
# Load shared workshop config first; a local AppBook .env can override it.
for candidate in (APP_DIR.parent / ".env", APP_DIR.parent / "app" / ".env", APP_DIR / ".env"):
    if candidate.exists():
...
class Settings:
    ...
    # Accept the lightweight workshop’s ORACLE_* variables so both demos use the same database.
    ora_user: str = os.environ.get("ORA_AGENT_USER") or os.environ.get("ORACLE_AGENT_USER", "AGENT")
    ora_password: str = os.environ.get("ORA_AGENT_PWD") or os.environ.get("ORACLE_AGENT_PASS", "AgentPw_2026")
    ora_dsn: str = os.environ.get("ORA_DSN") or os.environ.get("ORACLE_DSN", "localhost:1521/FREEPDB1")
    oracle_enabled: bool = os.environ.get("ORACLE_ENABLED", "1") not in {"0", "false", "False"}

    # Models (loaded into the database by the notebook).
    embed_model: str = os.environ.get("EMBED_MODEL") or os.environ.get("ONNX_EMBED_MODEL", "ALL_MINILM_L12_V2")
