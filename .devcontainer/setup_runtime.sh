...
# The app and notebook rotate across all configured OCI_GENAI_API_KEY[_N] slots.
...
for key_name in ["OCI_GENAI_API_KEY", *[f"OCI_GENAI_API_KEY_{i}" for i in range(2, 9)]]:
    if os.environ.get(key_name):
        text = set_kv(text, key_name, os.environ[key_name])
if os.environ.get("OCI_COMPARTMENT_ID"):
    text = set_kv(text, "OCI_COMPARTMENT_ID", os.environ["OCI_COMPARTMENT_ID"])
