...
# ── tool registry ──────────────────────────────────────────────────────────
def register_tool(name, fn, description, params, *, category="general",
                  synonyms=None, examples=None, when_to_use=""):
    ...
    db.x(f'''MERGE INTO agent_tools d USING (SELECT :n AS name FROM dual) s ON (d.name=s.name)
...
def retrieve_tools(query, k=6):
    return db.q(f'''SELECT name, description, tool_schema,
...
# ── doer tools ──────────────────────────────────────────────────────────────
def list_sources():
    rows = db.q("SELECT table_name FROM user_tables WHERE table_name IN "
...
def author_materialized_view(name, select_sql):
    ...
    db.ddl(f"CREATE MATERIALIZED VIEW {name} BUILD IMMEDIATE REFRESH COMPLETE ON DEMAND AS {select_sql}")
    return {"created": name}
...
def _job(job, action, repeat):
    try:
        ...
        pass
    db.x('''BEGIN DBMS_SCHEDULER.CREATE_JOB(job_name=>:n, job_type=>'PLSQL_BLOCK',
            job_action=>:a, repeat_interval=>:r, enabled=>TRUE); END;''', {"n": job, "a": action, "r": repeat})
...
def create_automation(name, select_sql, cadence_hours=24, description=""):
    ...
    if "error" in res:
        return res
    _job(job, f"BEGIN DBMS_MVIEW.REFRESH('{mv}', 'C'); END;", f"FREQ=HOURLY;INTERVAL={cadence_hours}")
    db.x("DELETE FROM agent_automations WHERE name=:n", {"n": name})
    db.x('''INSERT INTO agent_automations (name, description, artifact, job_name, cadence_hours, select_sql)
            VALUES (:n,:d,:a,:j,:c,:s)''',
         {"n": name, "d": description, "a": mv, "j": job, "c": cadence_hours, "s": select_sql})
    memory.capture_workflow(f"build automation {name}: {description}",
...
def list_automations():
    return db.q("SELECT name, artifact, job_name, cadence_hours, description FROM agent_automations ORDER BY created_at")


def automation_status():
    """Automations joined with their live DBMS_SCHEDULER state (enabled, last/next run)."""
    return db.q("""SELECT a.name, a.artifact, a.job_name, a.cadence_hours, a.description,
...
def save_skill(name, description, skill_md, tools_used, source_workflow_id=None, source_url=None):
    ...
    db.x(f'''MERGE INTO agent_skills d USING (SELECT :n AS name FROM dual) s ON (d.name=s.name)
...
def retrieve_skills(query, k=5):
    return db.q(f'''SELECT name, description, sha, source_url,
                VECTOR_DISTANCE(embedding, VECTOR_EMBEDDING({EMB} USING :q AS DATA), COSINE) dist
...
def load_skill(name):
    r = db.q("SELECT name, description, skill_md, tools_used, sha, source_url FROM agent_skills WHERE name=:n", {"n": name})
    return r[0] if r else {"error": "no such skill"}


def promote_workflow_to_skill(workflow_id):
    wf = db.q("SELECT RAWTOHEX(id) id, intent, steps, tools_used FROM agent_workflow WHERE RAWTOHEX(id)=:i",
              {"i": workflow_id.upper()})
    if not wf:
    ...
    try:
        ...
        out = (client.chat.completions.create(model=MODEL, messages=[{"role": "user", "content":
                  f"Distil into a skill. Intent: {wf['INTENT']}. Steps: {wf['STEPS']}. "
...
def harvest_skills(min_occurrences=3, recency_days=30):
    cands = db.q('''SELECT RAWTOHEX(id) id FROM agent_workflow
...
def register_default_tools():
    register_tool("list_sources", lambda: list_sources(),
                  ...
                  category="explore", synonyms=["schema", "tables"], when_to_use="before writing SQL")
    register_tool("run_sql", lambda sql: run_sql(sql),
                  "Run a read-only SELECT and return rows.", {"sql": "string"},
                  category="data", synonyms=["query", "analyze"], when_to_use="to answer an analytical question")
    register_tool("create_automation", lambda name, select_sql, cadence_hours=24, description="":
                  create_automation(name, select_sql, cadence_hours, description),
                  ...
                  category="automation", synonyms=["schedule", "recurring", "daily report"])
    register_tool("search_memory", lambda query: memory.recall(query),
                  "Search durable long-term memory by meaning.", {"query": "string"}, category="memory")
    register_tool("recall_workflow", lambda query: memory.recall_workflow(query),
    ...
    register_tool("find_skill", lambda query: build_skill_manifest(query),
                  "List relevant skills (the level-1 manifest).", {"query": "string"}, category="skills")
    register_tool("load_skill", lambda name: load_skill(name),
                  "Load a skill's full SKILL.md body.", {"name": "string"}, category="skills")
    register_tool("list_automations", lambda: automation_status(),
                  "List existing automations with their schedule state.", {}, category="automation")
    register_tool("toggle_automation", lambda name, enable=True: toggle_automation(name, enable),
                  "Start (enable) or stop (disable) an automation's schedule.",
                  {"name": "string", "enable": "boolean"}, category="automation",
                  synonyms=["start", "stop", "pause", "resume", "enable", "disable"])
    register_tool("run_automation_now", lambda name: run_automation_now(name),
                  "Refresh an automation now and return its latest rows.", {"name": "string"},
                  category="automation", synonyms=["refresh", "run now", "execute"])
    register_tool("remember_fact", lambda fact: (memory.remember(fact), {"saved": fact[:60]})[1],
