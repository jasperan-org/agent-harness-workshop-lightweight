...
def write(path: str, content) -> None:
    ...
    db.x('''MERGE INTO agent_scratch d USING (SELECT :p AS path FROM dual) s ON (d.path = s.path)
            WHEN MATCHED THEN UPDATE SET content=:c, is_dir='N', promoted='N', updated_at=SYSTIMESTAMP
            WHEN NOT MATCHED THEN INSERT (path, content) VALUES (:p, :c)''', {"p": _abs(path), "c": data})
...
def listing(root: str = "/"):
    ...
    return [r["PATH"] for r in db.q(
        "SELECT path, updated_at FROM agent_scratch WHERE path LIKE :pre AND is_dir='N' ORDER BY path", {"pre": pre})]
