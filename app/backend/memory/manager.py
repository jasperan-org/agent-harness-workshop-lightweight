...
from oci_key_rotation import KeyRotator, load_oci_keys, make_rotating_oamp_llm
...
def build_extraction_llm():
    ...
    if LLM_PROVIDER == "oci" and _OCI_ROTATOR and OCI_GENAI_ENDPOINT:
        try:
            llm = make_rotating_oamp_llm(LLM_MODEL, OCI_GENAI_ENDPOINT, _OCI_ROTATOR)
            print(f"[memory] OAMP extraction LLM = OCI {LLM_MODEL} @ {OCI_GENAI_ENDPOINT} "
                  f"keys={len(_OCI_ROTATOR)} start_key={_OCI_ROTATOR.current_index() + 1}")
            return llm
