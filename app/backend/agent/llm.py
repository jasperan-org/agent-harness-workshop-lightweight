...
Mirrors the notebook's §3.3 wiring but with two production-grade additions:
  ...
  1. OCI is *attempted first* whenever one or more OCI_GENAI_API_KEY[_N]
     credentials are present and the endpoint normalizes to a valid
     OpenAI-compatible URL. If the very first
...
from oci_key_rotation import KeyRotator, call_with_failover, load_oci_keys

_OCI_KEYS = load_oci_keys() if LLM_PROVIDER == "oci" else []
_OCI_ROTATOR = KeyRotator(_OCI_KEYS) if _OCI_KEYS else None
...
class _RotatingCompletions:
    """OpenAI-compatible completions surface backed by rotating OCI keys."""

    def create(self, **kwargs):
        def make_client(key: str):
            options = {"api_key": key, "base_url": OCI_GENAI_ENDPOINT}
        ...
        return call_with_failover(
            ...
            _OCI_ROTATOR,
            on_event=lambda message: print(f"[llm] {message}"),
        )
...
class LlmRouter:
    ...
    def _build(self) -> None:
        ...
        if LLM_PROVIDER == "oci" and _OCI_ROTATOR and OCI_GENAI_ENDPOINT:
            try:
        ...
        if self.primary is None:
            raise RuntimeError(
                ...
                "and/or OCI_GENAI_API_KEY[_N] + OCI_GENAI_ENDPOINT in .env."
            )

    def _log_oci_setup(self) -> None:
        print(f"[llm] OCI primary configured: model={LLM_MODEL} endpoint={OCI_GENAI_ENDPOINT} "
              f"keys={len(_OCI_ROTATOR) if _OCI_ROTATOR else 0} "
              f"start_key={( _OCI_ROTATOR.current_index() + 1) if _OCI_ROTATOR else 'n/a'}")
